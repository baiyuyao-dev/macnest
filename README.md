...
| 模块 | 核心功能 |
|------|---------|
| **仪表盘** | 全局运行状态概览、服务状态卡片（一键启停/重启/查看日志）、快速访问书签、实时资源统计、Tmux 会话数、SSH 终端连接数 |
| **服务管理** | 本地进程托管（配置启动命令、工作目录、环境变量），自动端口检测（`lsof`），重启策略（始终/失败/不重启），应用重启后自动恢复运行中的服务，内存日志存储，操作动效，实时日志查看器 |
| **Docker 管理** | 容器启停/重启/删除/重建（保留数据卷），实时资源监控（CPU/内存/网络/IO），内置 WebSocket 终端进入容器执行命令，镜像管理（列表/删除/清理/拉取），数据卷管理，网络管理，系统磁盘占用分析 |
| **SSH 终端** | 连接管理（密码/密钥认证），树形分组，多标签页，内置 XTerm.js 终端（支持选区保留、Cmd+C 复制），集成分栏式 SFTP 文件管理（上传/下载/删除/重命名/新建文件夹/右键菜单），传输进度显示与取消，终端路径自动同步 SFTP 路径 |
| **Tmux 会话** | 创建/删除/重命名/复制会话，工作目录编辑与路径验证，内置 PTY 终端 attach（支持鼠标滚轮、选区复制），支持在 [Ghostty](https://ghostty.org/) 中打开，自动生成兼容配置文件 |
| **服务导航** | 常用 URL 书签管理，树形分组，网格/列表视图，25 种图标可选，点击统计，可选关联本地服务，Safari 书签一键导入 |
...
4. 支持树形分组管理多个连接
5. 双击连接即可在右侧终端中打开
6. 点击「SFTP」按钮进入文件管理界面，支持拖拽上传、右键菜单操作
7. 终端与 SFTP 路径自动同步，点击「同步到终端」可快速切换目录

### 管理 Tmux 会话

1. 点击左侧菜单「Tmux」
2. 点击「新建会话」，输入名称和可选工作目录
3. 点击会话卡片即可在右侧 attach 终端
4. 支持复制会话配置、编辑工作目录、在 Ghostty 中打开
5. 内置终端支持鼠标滚轮浏览历史、选区复制
...
│   │   ├── safari_bookmarks.rs   # Safari 书签导入
...
| `tmux_sessions` | Tmux 会话映射（显示名、工作目录） |
...
首次导入需要授予 MacNest 「完全磁盘访问权限」：
系统设置 → 隐私与安全性 → 完全磁盘访问权限 → 添加 MacNest
...
### v0.2.0 (2026-05-28)
- SSH 终端路径自动同步 SFTP 路径，支持一键将当前目录发送到终端
- Docker 增强：镜像拉取、系统磁盘分析、容器内 WebSocket 终端
- Tmux 剪贴板支持：选区复制、Cmd+C 复制选中文本

### v0.2.0 (2026-05-27)
- SFTP 文件列表右键菜单：进入目录、下载、重命名、复制路径、删除、新建文件夹、上传、刷新

### v0.2.0 (2026-05-26)
- 服务管理：内存日志存储、操作动效、实时日志查看器（支持自动滚动、5000 条历史）

### v0.2.0 (2026-05-25)
- Tmux 会话复制、工作目录编辑与路径验证
- SSH 侧边栏选中交互优化、SFTP 工具栏自适应与配色统一
- 终端支持选区保留和 Cmd+C 复制选中文本
- Tmux 工作目录支持、Ghostty 打开修复（AppleScript 方式）
- 主题设置应用启动时自动加载
...
### v0.2.0 (2026-05-20)
- 修复菜单栏图标显示与交互问题
- 多角色代码审查修复：安全加固、内存泄漏修复、UI 优化
- 移除书签健康检查、服务页面精简、仪表盘指标标注

### v0.2.0 (2026-05-19)
- 多会话 SSH 终端：标签页管理、SFTP 面板、保活机制
- SFTP 路径同步、传输进度显示优化

### v0.2.0 (2026-05-18)
- Tmux 会话管理前后端完整实现
- SFTP 文件管理器：上传、下载、拖拽、删除、重命名、新建文件夹
- Safari 书签一键导入

### v0.1.0 (2026-05-17)
- 项目初始化：SSH 终端、连接管理、基础架构搭建
